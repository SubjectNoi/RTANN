        ...
        // for (int c = 0; c < _coarse_grained_cluster_num; c++) {
            for (int d = 0; d < dim_pair; d++) {
                ...
                for (int n = 0; n < num_sphere_per_dim_pair; n++) {
                    ...
                    centers[_c * dim_pair * num_sphere_per_dim_pair + d * num_sphere_per_dim_pair + n] = make_float3(x, y, 1.0 * (c * _D + 2 * d + 1));
                    ...
                    // radius[_c * dim_pair * num_sphere_per_dim_pair + d * num_sphere_per_dim_pair + n] = static_cast<float>(sqrt(beta * beta + x * x + y * y));
                    prim_idx++;
                }
            }
        }
