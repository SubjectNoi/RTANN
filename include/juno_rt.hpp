        ...
        for (int c = 0; c < _coarse_grained_cluster_num; c++) {
            for (int d = 0; d < dim_pair; d++) {
                ...
                for (int n = 0; n < num_sphere_per_dim_pair; n++) {
                    float x = (1.0 * _codebook_entry[c][d][n][0]) / 0.4;
                    float y = (1.0 * _codebook_entry[c][d][n][1]) / 0.4;
                    // float factor = 0.01 * std::min(x, y);
                    // if (c == 432) {
                    //     printf("Prim %d, c %d, d %d, bit %d: (%.6f, %.6f, %.6f)\n", prim_idx, c, d, n, x, y, 1.0 * (c * 128 + 2 * d + 1));
                    // }
                    centers[c * dim_pair * num_sphere_per_dim_pair + d * num_sphere_per_dim_pair + n] = make_float3(x, y, 1.0 * (c * _D + 2 * d + 1));
                    ...
                    radius[c * dim_pair * num_sphere_per_dim_pair + d * num_sphere_per_dim_pair + n] = static_cast<float>(sqrt(_radius * _radius + x * x + y * y));
                    prim_idx++;
                }
            }
        }
